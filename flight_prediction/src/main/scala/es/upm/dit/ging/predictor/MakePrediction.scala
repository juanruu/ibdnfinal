...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaStreamWriter = kafkaOutput.writeStream
      .format("kafka")
      .option("kafka.bootstrap.servers", "kafkaibdn:9092")
      .option("topic", "flight-delay-ml-response")
      .option("checkpointLocation", "/tmp/kafka-checkpoint-response")
      .outputMode("append")
      .start()

finalPredictions.writeStream
  .format("json")
  .option("path", "hdfs://namenode:8020/user/predictions")
